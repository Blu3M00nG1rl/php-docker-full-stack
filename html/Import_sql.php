    ...
    while (false !== ($filename = readdir($handle))) {
        ...
        if ($filename != "." && $filename != "..") {
            ...
            while ($data = $sql2->fetch())
            {
                ...
                $json_data = file_get_contents('prices/cc_price_'.$symbol.'.json');
                // Convertir le JSON en tableau associatif
                $data = json_decode($json_data, true);
                // Parcourir les données et les insérer dans la base de données
                foreach ($data as $coin => $values) {
                    $price = $values['EUR']['PRICE'];
                    $volume = $values['EUR']['TOTALVOLUME24HTO'];
                    $market_cap = $values['EUR']['MKTCAP'];
                    if ($market_cap == 0) {
                        $market_cap = $values['EUR']['CIRCULATINGSUPPLYMKTCAP'];
                    } 
    ...
    while (false !== ($filename = readdir($handle))) {
        ...
        if ($filename != "." && $filename != "..") {
            ...
            while ($data = $sql2->fetch())
            {
                $id = $data['id'];
                // Charger le fichier JSON
                $json_data = file_get_contents('prices_1an/cc_price_'.$symbol.'.json');
                // Convertir le JSON en tableau associatif
                $data = json_decode($json_data, true);
                // Parcourir les données et les insérer dans la base de données
                $price = $data['EUR'];
    ...
    while (false !== ($filename = readdir($handle))) {
        ...
        if ($filename != "." && $filename != "..") {
            ...
            while ($data = $sql2->fetch())
            {
                $id = $data['id'];
                // Charger le fichier JSON
                $json_data = file_get_contents('prices_30j/cc_price_'.$symbol.'.json');
                // Convertir le JSON en tableau associatif
                $data = json_decode($json_data, true);
                // Parcourir les données et les insérer dans la base de données
                $price = $data['EUR'];
    ...
    while (false !== ($filename = readdir($handle))) {
        ...
        if ($filename != "." && $filename != "..") {
            ...
            while ($data = $sql2->fetch())
            {
                $id = $data['id'];
                // Charger le fichier JSON
                $json_data = file_get_contents('prices_hier/cc_price_'.$symbol.'.json');
                // Convertir le JSON en tableau associatif
                $data = json_decode($json_data, true);
                // Parcourir les données et les insérer dans la base de données
                $price = $data['EUR'];
...
echo "<a href='index.php'><h2>Retour à l'acceuil</h2></a>";
